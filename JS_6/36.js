/** ЗАДАЧА 36 - Проверка сортировки массива
 *
...
function arraySortInfo(inputArray) {
    if (!inputArray.every(Number.isInteger)) return "Некоторые элементы не являются числами"
    if (inputArray.every((element, index, array) => !index || array[index-1] <= element)) {
        return "Массив отсортирован по возрастанию"
    } else if (inputArray.every((element, index, array) => !index || array[index-1] >= element)) {
